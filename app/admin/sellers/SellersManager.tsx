...
import type { SerializedSeller } from "@/lib/sellers"
...
const emptyForm: FormData = { name: "", email: "", phone: "", cpf: "", notes: "" }

export default function SellersManager({
  initialData,
  initialMeta,
}: {
  initialData: SerializedSeller[]
  initialMeta: Meta
}) {
  ...
  async function fetchSellers(page = 1) {
    ...
    try {
      ...
      setFeedback({ type: "error", message: "Erro ao buscar vendedores." })
    } finally {
      setLoading(false)
    }
  }
  ...
  function openEdit(seller: SerializedSeller) {
    setForm({
      name: seller.name,
      email: seller.email ?? "",
      phone: seller.phone ?? "",
  ...
  async function handleSubmit() {
    ...
    try {
      const url = editingId ? `/api/admin/sellers/${editingId}` : "/api/admin/sellers"
      ...
      setFeedback({ type: "success", message: editingId ? "Vendedor atualizado." : "Vendedor criado." })
      closeForm()
      fetchSellers(meta.page)
    } catch {
  ...
  async function handleDeactivate(id: string) {
    try {
      ...
      setFeedback({ type: "success", message: "Vendedor desativado." })
      fetchSellers(meta.page)
    } catch {
      setFeedback({ type: "error", message: "Erro de conexão." })
    }
            ...
            <tbody>
              {sellers.map((s) => (
                <tr key={s.id} className="border-b border-white/5 hover:bg-white/5">
                  ...
                  <td className="px-4 py-4 text-gray-300 font-mono text-xs">{s.cpf ?? "—"}</td>
              ...
              {sellers.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-gray-500">Nenhum vendedor encontrado.</td>
                </tr>
              )}
            </tbody>
          </table>
