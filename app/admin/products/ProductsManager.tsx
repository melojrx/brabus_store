...
import { FolderTree, Loader2, Package, Pencil, Plus, Trash2, X } from "lucide-react"
...
export default function ProductsManager({ initialProducts, categories }: ProductsManagerProps) {
  ...
  const totalVariantStock = form.variants.reduce((sum, variant) => sum + (Number.parseInt(variant.stock || "0", 10) || 0), 0)
  const marginPreview =
  ...
  function removeVariant(index: number) {
    setForm((prev) => ({
      ...prev,
      variants: prev.variants.length === 1 ? prev.variants : prev.variants.filter((_, variantIndex) => variantIndex !== index),
    }))
  }
  ...
  async function handleSave() {
    ...
    if (!form.name || !form.price || !form.costPrice || !form.categoryId) {
      setError("Preencha nome, preço de venda, preço de custo e subcategoria.")
      return
    }
    ...
    setSaving(true)
    try {
      const normalizedImages = form.images.split(",").map((item) => item.trim()).filter(Boolean)
      ...
      <div className="mb-8 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500">Subcategoria, preço de custo e variantes reais.</p>
          </div>
        </div>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                      ...
                      className="rounded-sm border border-red-500/30 p-1.5 text-red-400 transition-colors hover:bg-red-500/10"
                    >
      ...
      {drawerOpen ? (
        <div className="fixed inset-0 z-50 flex">
          <button
            ...
            onClick={closeDrawer}
            className="flex-1 border-0 bg-black/60 p-0 backdrop-blur-sm"
          />
          ...
          <div className="relative z-10 ml-auto flex h-full w-full max-w-3xl flex-col border-l border-zinc-800 bg-black shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-950 px-6 py-4">
              <div>
                ...
                <p className="mt-1 text-xs uppercase tracking-[0.2em] text-zinc-500">Catálogo com variantes e custo</p>
              </div>
            ...
            <div className="flex-1 space-y-6 overflow-y-auto px-6 py-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Nome" htmlFor="product-name">
                  <input id="product-name" className={inputCls} value={form.name} onChange={(event) => setField("name", event.target.value)} />
                </Field>

                <Field label="Slug" htmlFor="product-slug" hint="gerado automaticamente">
                  <input id="product-slug" className={`${inputCls} font-mono`} value={form.slug} onChange={(event) => setField("slug", event.target.value)} />
                </Field>
              </div>

              <Field label="Descrição" htmlFor="product-description">
                <textarea id="product-description" className={inputCls} rows={3} value={form.description} onChange={(event) => setField("description", event.target.value)} />
              </Field>

              <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                <Field label="Preço de Venda" htmlFor="product-price">
                  <input id="product-price" className={inputCls} type="number" step="0.01" value={form.price} onChange={(event) => setField("price", event.target.value)} />
                </Field>

                <Field label="Preço de Custo" htmlFor="product-cost-price">
                  <input id="product-cost-price" className={inputCls} type="number" step="0.01" value={form.costPrice} onChange={(event) => setField("costPrice", event.target.value)} />
                </Field>

                <Field label="Estoque Total" htmlFor="product-total-stock" hint="somado das variantes">
                  <input id="product-total-stock" className={`${inputCls} bg-zinc-950 text-zinc-400`} value={String(totalVariantStock)} readOnly />
                </Field>
                ...
                <Field label="Margem Bruta" htmlFor="product-margin" hint="preview">
                  <input
                    ...
                    value={marginPreview != null && Number.isFinite(marginPreview) ? `${marginPreview.toFixed(1)}%` : "—"}
                    readOnly
                  />
                </Field>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Imagem(s)" htmlFor="product-images" hint="URLs separadas por vírgula">
                  <input id="product-images" className={inputCls} value={form.images} onChange={(event) => setField("images", event.target.value)} placeholder="/produto1.jpg, /produto2.jpg" />
                </Field>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Peso KG (frete)" htmlFor="product-weight-kg">
                  <input id="product-weight-kg" className={inputCls} type="number" step="0.01" value={form.weightKg} onChange={(event) => setField("weightKg", event.target.value)} />
                </Field>
              </div>

              {showFashionFields ? (
                <Field label="Gênero" htmlFor="product-gender">
                  <select id="product-gender" className={inputCls} value={form.gender} onChange={(event) => setField("gender", event.target.value)}>
                  ...
                  <div className="mt-4 space-y-4">
                    {form.variants.map((variant, index) => (
                      <div key={variant.id ?? `variant-${index}`} className="rounded-sm border border-zinc-800 bg-black/30 p-4">
                        ...
                        <div className={`grid gap-4 ${selectedCategory.supportsSize || selectedCategory.supportsColor || selectedCategory.supportsFlavor ? "lg:grid-cols-3" : "lg:grid-cols-3"}`}>
                          <Field label="Nome da Variante" htmlFor={`variant-name-${index}`} hint="interno">
                            <input id={`variant-name-${index}`} className={inputCls} value={variant.name} onChange={(event) => setVariantField(index, "name", event.target.value)} />
                          </Field>

                          <Field label="SKU" htmlFor={`variant-sku-${index}`} hint="opcional">
                            <input id={`variant-sku-${index}`} className={inputCls} value={variant.sku} onChange={(event) => setVariantField(index, "sku", event.target.value)} />
                          </Field>

                          <Field label="Estoque" htmlFor={`variant-stock-${index}`}>
                            <input id={`variant-stock-${index}`} className={inputCls} type="number" value={variant.stock} onChange={(event) => setVariantField(index, "stock", event.target.value)} />
                          </Field>
                        </div>
                          ...
                          <div className="mt-4 grid gap-4 lg:grid-cols-3">
                            {selectedCategory.supportsSize ? (
                              <Field label="Tamanho" htmlFor={`variant-size-${index}`}>
                                <input id={`variant-size-${index}`} className={inputCls} value={variant.size} onChange={(event) => setVariantField(index, "size", event.target.value)} placeholder="P, M, G..." />
                              </Field>
                            ) : null}

                            {selectedCategory.supportsColor ? (
                              <Field label="Cor" htmlFor={`variant-color-${index}`}>
                                <input id={`variant-color-${index}`} className={inputCls} value={variant.color} onChange={(event) => setVariantField(index, "color", event.target.value)} placeholder="Preto" />
                              </Field>
                            ) : null}

                            {selectedCategory.supportsFlavor ? (
                              <Field label="Sabor" htmlFor={`variant-flavor-${index}`}>
                                <input id={`variant-flavor-${index}`} className={inputCls} value={variant.flavor} onChange={(event) => setVariantField(index, "flavor", event.target.value)} placeholder="Chocolate" />
                              </Field>
                            ) : null}
                          </div>
                        ) : null}
              ...
              <div className="flex flex-wrap gap-5 pt-2">
                ...
                <label className="flex items-center gap-2 text-sm font-medium text-white">
                  <input type="checkbox" checked={form.featured} onChange={(event) => setField("featured", event.target.checked)} className="h-4 w-4 accent-[var(--color-primary)]" />
                  Destaque
                </label>
                <label className="flex items-center gap-2 text-sm font-medium text-white">
                  <input type="checkbox" checked={form.isNew} onChange={(event) => setField("isNew", event.target.checked)} className="h-4 w-4 accent-[var(--color-primary)]" />
                  Novo
                </label>
              </div>
            </div>
