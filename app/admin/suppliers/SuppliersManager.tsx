...
import type { SerializedSupplier } from "@/lib/suppliers"
...
const emptyForm: FormData = {
  ...
  stateRegistration: "", contactName: "", addressStreet: "", addressNumber: "",
...
export default function SuppliersManager({
  ...
  initialData: SerializedSupplier[]
  initialMeta: Meta
}) {
  ...
  async function fetchSuppliers(page = 1) {
    ...
    try {
      ...
      setFeedback({ type: "error", message: "Erro ao buscar fornecedores." })
    } finally {
      setLoading(false)
    }
  }
  ...
  function openEdit(supplier: SerializedSupplier) {
    setForm({
      name: supplier.name,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ ...form, personType: form.personType || null }),
      })
      ...
      setFeedback({ type: "success", message: editingId ? "Fornecedor atualizado." : "Fornecedor criado." })
      closeForm()
  ...
  async function handleDeactivate(id: string) {
    try {
      ...
      setFeedback({ type: "success", message: "Fornecedor desativado." })
      fetchSuppliers(meta.page)
    } catch {
      setFeedback({ type: "error", message: "Erro de conexão." })
    }
    ...
    <div className="space-y-6">
        ...
        <form onSubmit={handleSearch} className="flex items-center gap-2 flex-1 min-w-[200px]">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Buscar por nome, email, contato..."
      ...
      {showForm && (
        <div className="rounded-sm border border-white/5 bg-zinc-900 p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="label-admin">Contato</label>
              <input type="text" value={form.contactName} onChange={(e) => updateField("contactName", e.target.value)} className="input-admin" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="md:col-span-3">
              ...
              <input type="text" value={form.notes} onChange={(e) => updateField("notes", e.target.value)} className="input-admin" />
            </div>
          </div>
            ...
            <tbody>
              {suppliers.map((s) => (
                <tr key={s.id} className="border-b border-white/5 hover:bg-white/5">
                  ...
                  <td className="px-4 py-4 text-gray-300">{s.contactName ?? "—"}</td>
                  <td className="px-4 py-4 text-gray-300">{s.email ?? "—"}</td>
                  <td className="px-4 py-4 text-gray-300">{s.phone ?? "—"}</td>
                  <td className="px-4 py-4 text-gray-300 font-mono text-xs">{formatDoc(s.cpf, s.cnpj)}</td>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => openEdit(s)} className="rounded-sm border border-white/10 p-1.5 text-gray-400 hover:border-white/30 hover:text-white transition-colors" title="Editar">
                        <Pencil className="h-3.5 w-3.5" />
                      </button>
                      {s.active && (
                        <button type="button" onClick={() => handleDeactivate(s.id)} className="rounded-sm border border-red-500/30 p-1.5 text-red-300 hover:bg-red-500/10 transition-colors" title="Desativar">
              ...
              {suppliers.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-4 py-10 text-center text-gray-500">Nenhum fornecedor encontrado.</td>
                </tr>
              )}
            </tbody>
          </table>
